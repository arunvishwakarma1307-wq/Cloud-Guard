...
class AppLockController extends ChangeNotifier {
  ...
  static const String _lockedUntilKey = 'cloud_guard_app_lock_locked_until';
  ...
  Future<void> _load() async {
    try {
      ...
      _failedAttempts = preferences.getInt(_failedAttemptsKey) ?? 0;

      final lockedUntilMilliseconds = preferences.getInt(_lockedUntilKey);
  ...
  AppLockVerifyResult verifyPin(String pin) {
    ...
    if (pin == _pin) {
      ...
      _persist();
      notifyListeners();
      return const AppLockVerifyResult(isValid: true, isLockedOut: false);
    }
  ...
  Future<void> _persist() async {
    try {
      ...
      if (_pin == null) {
        ...
        await preferences.setString(_pinKey, _pin!);
      }

      await preferences.setInt(_failedAttemptsKey, _failedAttempts);
      ...
      // App lock state remains usable for the current session if persistence fails.
    }
  }
}
